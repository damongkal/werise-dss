...
366.,   0.";
    ...
    {
        $step = _opt(sysoptions::_ORYZA_INTERVAL);
        $ret = array();
        if ($step === 1) {
            // daily   
            $refdate = DateTime::createFromFormat('Y-m-d', ($this->year) . '-01-01');
            while ($this->year >= intval($refdate->format('Y'))) {
                $ret[] = $refdate->format('z') + 1;
                $refdate->add(new DateInterval('P1D'));
            }
        } else {
            if (in_array($step, array(5, 15))) {
                for ($month = 1; $month <= $this->sttime_limit; $month++) {
                    $refdate = DateTime::createFromFormat('Y-m-d', $this->year . '-' . $month . '-01');
                    $ret[] = ($refdate->format('z') + 1);
                    $refdate->add(new DateInterval("P" . ($step - 1) . "D"));
                    $ret[] = ($refdate->format('z') + 1);
                    if ($step === 5) {
                        for ($i = 1; $i < 5; $i++) {
                            $refdate->add(new DateInterval('P5D'));
                            $ret[] = ($refdate->format('z') + 1);
                        }
                    }
                }
            } else {
                throw new Exception("Invalid interval value ({$step}) for STTIME. Check system options");
            }
        }
        return $ret;
    }
